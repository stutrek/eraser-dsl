...
import { getLanguageServer } from './languageServer.js';
...
export const getClientConfiguration = (language: string): LanguageClientConfig => {
  // vite does not extract the worker properly if it is URL is a variable
  const lsWorker = getLanguageServer();
